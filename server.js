...
app.post('/register', async (req, res) => {
    ...
    if (login == null || password == null || login === '' || password === '') {
        console.error('Логин или пароль отсутствуют или пустые:', { login, password });
        return res.status(400).json({ success: false, message: 'Логин или пароль не могут быть пустыми или null' });
    }
    ...
    if (login.trim().toLowerCase() === 'null') {
        console.error('Логин содержит "null":', { login });
        return res.status(400).json({ success: false, message: 'Логин не может содержать "null"' });
    }
...
app.listen(port, () => {
    console.log(`Сервер запущен на порту ${port}`);
>>>>>>> d5173e020a6f272f315fee81e9afbb4c73ef8d56
});